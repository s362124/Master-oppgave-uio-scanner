...
- passes: `2`
- bandwidth: `10M`
- DNS discovery on port 53: UDP probe for `example.com`; RD is off unless `--dns-rd` is set
- full postprocess pipeline: enabled (banner grab + SQLite + report)
- DNS follow-up in the full pipeline uses RD and `version.bind` by default unless disabled with `--pipeline-no-dns-rd` or `--pipeline-no-dns-version-bind`
...
```bash
sudo python zmap_scan_all.py --bandwidth 8M --passes 3 --seed 1 --seed-step 1000 --zmap-pass-timeout-sec 12000
```
...
For scheduled runs, use the `systemd` timer/service setup documented in
`automation/README_AUTOMATION.md`.

Automation templates in this repository are configured for every 8 hours.
Scheduled automation alternates between a `high_coverage` run and a `balanced` run.
