...
def normalize_identity(service: str, banner: str, software: str, extra: str) -> Tuple[str, str, str]:
    """
    Extract a normalized (product, version, status) tuple for a service.
    The raw banners table remains unchanged; these values are for derived inventory tables.
    """
    if service == "ssh":
        status, product, version = classify_ssh(software or banner or "")
        return product, version, status

    if service == "ftp":
        status, product, version = classify_ftp(banner or software or "")
        return product, version, status

    if service == "vnc":
        status, product, version = classify_vnc(banner or software or "")
        return product, version, status

    if service == "dns":
        text = (software or "").strip()
        if not text:
            return "", "", "unknown"
        m = re.search(r"^(PowerDNS(?: Authoritative Server)?)\s+([0-9]+(?:\.[0-9]+)+)", text, re.IGNORECASE)
        if m:
            return m.group(1), m.group(2), "unknown"
        m = re.search(r"^(dnsmasq)-([0-9]+(?:\.[0-9]+)+)", text, re.IGNORECASE)
        if m:
            return m.group(1), m.group(2), "unknown"
        if re.match(r"^[0-9]+\.[0-9]+", text):
            return "BIND", text, "unknown"
        return "dns", text, "unknown"

    if service == "ssh_auth":
        return "ssh_auth", "", "unknown"

    return "", "", "unknown"


def rebuild_inventory_tables(conn: sqlite3.Connection, rows: List[Tuple]) -> None:
    """
    Build derived inventory tables so the database exposes normalized product/version
    data without modifying the raw banners table.
    """
    conn.execute("drop table if exists service_inventory")
    conn.execute("drop table if exists host_inventory")
    ...
    conn.execute("create index idx_service_inventory_ip on service_inventory(ip)")
    conn.execute("create index idx_service_inventory_run on service_inventory(run_id)")
    ...
    conn.execute("create index idx_host_inventory_ip on host_inventory(ip)")

    service_rows = []
    host_rows: Dict[str, Dict[str, object]] = {}

    for row in rows:
        run_id, ts, ip, port, service, banner, software, extra, error = row
        product, version, status = normalize_identity(service, banner or "", software or "", extra or "")
        service_rows.append(
            (
                run_id or "",
                ts or "",
        ...
        host = host_rows.setdefault(
            ip,
            {
                ...
                "run_ids": set(),
                "services": set(),
                "ftp": {"present": 0, "product": "", "version": "", "status": ""},
                "ssh": {"present": 0, "product": "", "version": "", "status": ""},
                "dns": {"present": 0, "product": "", "version": "", "status": ""},
                "vnc": {"present": 0, "product": "", "version": "", "status": ""},
            },
        )
        if run_id:
            host["run_ids"].add(run_id)
        if service in ("ftp", "ssh", "dns", "vnc"):
            host["services"].add(service)
            current = host[service]
            should_replace = (
                current["present"] == 0
                or (version and not current["version"])
                or (product and not current["product"])
                or (status != "unknown" and current["status"] == "unknown")
            )
            if should_replace:
                host[service] = {
                    "present": 1,
    ...
    conn.executemany(
        ...
        insert into service_inventory (
            run_id, ts, ip, port, service, product, version, status, banner, software, extra, error
        ) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        service_rows,
    )
    ...
    host_insert_rows = []
    for _, host in sorted(host_rows.items()):
        services_running = ",".join(sorted(host["services"]))
        run_ids = ",".join(sorted(host["run_ids"]))
        host_insert_rows.append(
            (
                host["ip"],
                run_ids,
                len(host["services"]),
                services_running,
                host["ftp"]["present"], host["ftp"]["product"], host["ftp"]["version"], host["ftp"]["status"],
                host["ssh"]["present"], host["ssh"]["product"], host["ssh"]["version"], host["ssh"]["status"],
                host["dns"]["present"], host["dns"]["product"], host["dns"]["version"], host["dns"]["status"],
                host["vnc"]["present"], host["vnc"]["product"], host["vnc"]["version"], host["vnc"]["status"],
            )
        )
    ...
    conn.executemany(
        ...
        insert into host_inventory (
            ip, run_ids, service_count, services_running,
            ftp_present, ftp_product, ftp_version, ftp_status,
            ssh_present, ssh_product, ssh_version, ssh_status,
            dns_present, dns_product, dns_version, dns_status,
            vnc_present, vnc_product, vnc_version, vnc_status
        ) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        host_insert_rows,
    )
...
def main() -> int:
    ...
    inventory_rows = fetch_all(
        conn,
        "select run_id, ts, ip, port, service, banner, software, extra, error from banners",
    )
    rebuild_inventory_tables(conn, inventory_rows)

    # Risk classification (heuristic)
    risk_counts = {}
    examples = []
    rows = [(ip, service, banner, software, extra) for _, _, ip, _, service, banner, software, extra, _ in inventory_rows]
