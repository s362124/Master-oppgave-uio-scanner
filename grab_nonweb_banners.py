...
def _path_match_patterns(path: Path) -> List[str]:
    """
    Build a small set of patterns that can match both relative and absolute
    path forms for the same logical run artifact.
    """
    patterns = {
        str(path),
        str(path.resolve()),
...
def delete_existing_logical_run(
    ...
    """
    Make collector reruns idempotent for a given service/input/output pair by
    removing prior rows that represent the same logical collection step.
    """
    input_patterns = _path_match_patterns(input_path)
    output_patterns = _path_match_patterns(output_path)
    input_clause = " OR ".join(["input_file LIKE ?"] * len(input_patterns))
    output_clause = " OR ".join(["output_file LIKE ?"] * len(output_patterns))
    params: List[object] = [service, port, *input_patterns, *output_patterns]
    prior_run_ids = [
        row[0]
    ...
    conn.execute(f"DELETE FROM banners WHERE run_id IN ({placeholders})", prior_run_ids)
    conn.execute(f"DELETE FROM runs WHERE run_id IN ({placeholders})", prior_run_ids)
    conn.commit()
...
async def run_async(args: argparse.Namespace) -> int:
    ...
    if args.sqlite:
        ...
        init_db(db_conn)
        delete_existing_logical_run(db_conn, service, port, input_path, output_path)
        db_conn.execute(
            """
